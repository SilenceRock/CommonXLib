...
public abstract class AppPictureActivity extends AppActivity implements AppPictureInterface {
    ...
    public AppPictureActivity() {
    }

    protected void onSaveInstanceState(Bundle o) {
        if(this.d != null) {
            o.putString("d", this.d.getPath());
        }

        if(this.e != null) {
            o.putString("e", this.e.getPath());
        }
    ...
    protected void onRestoreInstanceState(Bundle s) {
        String u = s.getString("d");
        if(u != null && !u.equals("")) {
            this.d = this.getUri(u);
        }

        u = s.getString("e");
        if(u != null && !u.equals("")) {
            this.e = this.getUri(u);
        }
        ...
        if(UtilSDCard.isSDCardEnable()) {
            this.permission(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE}, new PermissionsResultAction() {
                public void onGranted() {
                    AppPictureActivity.this.startActivityForResult(new Intent("android.intent.action.PICK", MediaStore.Images.Media.EXTERNAL_CONTENT_URI), 4001);
                    AppPictureActivity.this.f = v;
                }

                public void onDenied(String permission) {
                    (new AlertDialog.Builder(AppPictureActivity.this.context)).setCancelable(false).setTitle(UtilApp.appName()).setMessage("请手动开启存储权限").setPositiveButton("否", (DialogInterface.OnClickListener)null).setNegativeButton("是", new DialogInterface.OnClickListener() {
    ...
    public void startCamera(final ImageView v, final boolean isFrontCamera) {
        if(UtilSDCard.isSDCardEnable()) {
            this.permission(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE,Manifest.permission.CAMERA}, new PermissionsResultAction() {
                public void onGranted() {
                    AppPictureActivity.this.startActivityForResult((new Intent("android.media.action.IMAGE_CAPTURE"))
                            .putExtra("output", AppPictureActivity.this.b())
                            .putExtra("android.intent.extras.CAMERA_FACING",isFrontCamera ? 1 : 0), 4002);
                    AppPictureActivity.this.f = v;
                }

                public void onDenied(String permission) {
                    (new AlertDialog.Builder(AppPictureActivity.this.context)).setCancelable(false).setTitle(UtilApp.appName()).setMessage("请手动开启存储/相机拍照权限").setPositiveButton("否", (DialogInterface.OnClickListener)null).setNegativeButton("是", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialogInterface, int i) {
                            UtilPermissionManager.show();
                        }
                    }).show();
    ...
    protected void onActivityResult(int q, int s, Intent t) {
        try {
            if(s == -1 && g.a(this)) {
    ...
    public Uri ri(File f) {
        ...
        Cursor c = this.getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, new String[]{"_id"}, "_data=? ", new String[]{fp}, (String)null);
        if(c != null && c.moveToFirst()) {
            ...
            v.put("_data", fp);
            return this.getContentResolver().insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, v);
        } else {
    ...
    private String d(Uri u) {
        ...
        Cursor c = this.managedQuery(u, j, (String)null, (String[])null, (String)null);
        if(c != null) {
