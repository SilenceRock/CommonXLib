...
/**
 * Created by Hang.Yang on 2018/8/17 15:03.
 * If you find a path with no obstacles, it probably doesn’t lead anywhere.
 * In LongCaiHaErBin
 */

@SuppressLint({"SdCardPath", "SimpleDateFormat"})
public abstract class AppV4PictureActivity extends AppV4Activity implements AppPictureInterface {
    ...
    public void startAlbum(final ImageView v) {
        if(UtilSDCard.isSDCardEnable()) {
            AppV4PictureActivity.this.startActivityForResult(new Intent("android.intent.action.PICK", MediaStore.Images.Media.EXTERNAL_CONTENT_URI), 4001);
            AppV4PictureActivity.this.f = v;
        } else {
    ...
    public void startCamera(final ImageView v, final boolean isFrontCamera) {
        if(UtilSDCard.isSDCardEnable()) {
            AppV4PictureActivity.this.startActivityForResult((new Intent("android.media.action.IMAGE_CAPTURE"))
                    .putExtra("output", AppV4PictureActivity.this.b())
