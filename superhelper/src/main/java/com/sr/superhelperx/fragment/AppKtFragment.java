...
/**
 * FileName: AppKtFragment
 * Author: hangY
 * Date: 2021/5/6 9:33
 * Description:
 */
public abstract class AppKtFragment extends Fragment implements AppInterface {
    protected Map<Class<? extends AppReceiver>, BroadcastReceiver> m = new HashMap();

    public AppKtFragment() {
    }
    ...
    public void delayed(long delayMillis, final OnDelayedEnd onDelayedEnd) {
        (new Handler() {
            public void handleMessage(Message msg) {
                try {
                    if(AppApplication.INSTANCE.currentActivity() == AppKtFragment.this.getActivity()) {
                        onDelayedEnd.onEnd();
                    }
                } catch (Exception var3) {
                    ;
