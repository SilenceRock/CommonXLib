...
@z*/
@TargetApi(11)
public abstract class AppV4Fragment extends Fragment implements AppInterface {
    protected Map<Class<? extends AppReceiver>, BroadcastReceiver> m = new HashMap();

    public AppV4Fragment() {
    }
    ...
    public void delayed(long delayMillis, final OnDelayedEnd onDelayedEnd) {
        (new Handler() {
            public void handleMessage(Message msg) {
                try {
                    if(AppApplication.INSTANCE.currentActivity() == AppV4Fragment.this.getActivity()) {
                        onDelayedEnd.onEnd();
                    }
                } catch (Exception var3) {
                    ;
