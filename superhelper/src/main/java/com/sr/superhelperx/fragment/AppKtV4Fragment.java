...
/**
 * FileName: AppKtV4Fragment
 * Author: hangY
 * Date: 2021/5/6 9:27
 * Description:
 */
public abstract class AppKtV4Fragment extends Fragment implements AppInterface {
    protected Map<Class<? extends AppReceiver>, BroadcastReceiver> m = new HashMap();

    public AppKtV4Fragment() {
    }
    ...
    public void delayed(long delayMillis, final OnDelayedEnd onDelayedEnd) {
        (new Handler() {
            public void handleMessage(Message msg) {
                try {
                    if(AppApplication.INSTANCE.currentActivity() == AppKtV4Fragment.this.getActivity()) {
                        onDelayedEnd.onEnd();
                    }
                } catch (Exception var3) {
                    ;
