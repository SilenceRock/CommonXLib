...
import com.sr.superhelperx.imagepicker.ImagePicker;
import com.sr.superhelperx.imagepicker.adapter.ImageFoldersAdapter;
import com.sr.superhelperx.imagepicker.adapter.ImagePickerAdapter;
import com.sr.superhelperx.imagepicker.data.MediaFile;
import com.sr.superhelperx.imagepicker.data.MediaFolder;
import com.sr.superhelperx.imagepicker.executors.CommonExecutor;
...
import com.sr.superhelperx.imagepicker.task.ImageLoadTask;
import com.sr.superhelperx.imagepicker.task.MediaLoadTask;
import com.sr.superhelperx.imagepicker.task.VideoLoadTask;
import com.sr.superhelperx.imagepicker.utils.DataUtil;
import com.sr.superhelperx.imagepicker.utils.PermissionUtil;
import com.sr.superhelperx.imagepicker.utils.Utils;
import com.sr.superhelperx.imagepicker.view.ImageFolderPopupWindow;
...
public class ImagePickerActivity extends BaseActivity implements ImagePickerAdapter.OnItemClickListener, ImageFoldersAdapter.OnImageFolderChangeListener {
    ...
    protected void initConfig() {
        ...
        mImageMaxCount = ConfigManager.getInstance().getImageMaxCount();
        mVideoMaxCount = ConfigManager.getInstance().getVideoMaxCount();
        SelectionManager.getInstance().setImageMaxCount(mImageMaxCount);
        SelectionManager.getInstance().setVideoMaxCount(mVideoMaxCount);
        //如果是多选模式，载入历史记录
        ...
        if (requestCode == REQUEST_PERMISSION_CAMERA_CODE) {
            if (grantResults.length >= 1) {
                ...
                if (cameraGranted && sdGranted) {
                    ...
                    //没有权限
                    UtilToast.show(getString(R.string.permission_tip));
//                    Toast.makeText(this, getString(R.string.permission_tip), Toast.LENGTH_SHORT).show();
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mediaFolderList.size() > 0){
                        ...
                        Toast.makeText(ImagePickerActivity.this,"没有找到视频/图片", Toast.LENGTH_SHORT).show();
                    }
    ...
    private void showNoChooseToast(){
        if (mVideoMaxCount == 0){
            UtilToast.show("最多选择" + mImageMaxCount + "张图片");
        }else {
            UtilToast.show("最多选择" + mImageMaxCount + "张图片," +  mVideoMaxCount + "个视频");
        }
    }
    ...
    private void updateCommitButton() {
        ...
        if (selectCount < mImageMaxCount + mVideoMaxCount) {
            mTvCommit.setEnabled(true);
            mTvCommit.setText(String.format(getString(R.string.confirm_msg), selectCount, mImageMaxCount + mVideoMaxCount));
            return;
        }
        if (selectCount == mImageMaxCount + mVideoMaxCount) {
            mTvCommit.setEnabled(true);
...
//        super.onActivityResult(requestCode, resultCode, data);
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAPTURE) {
                ...
                /**
                 * 2019/08/02  拍照后下次进入会保留选中状态，是因为拍照时保存了状态。
                 * 先加上这句设置
                 */
