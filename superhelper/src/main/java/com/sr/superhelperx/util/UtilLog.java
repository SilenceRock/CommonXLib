...
public class UtilLog {
    ...
    private static void printLog(int type, String tagStr, Object objectMsg) {
        ...
        stringBuilder.append("[ (").append(className).append(":").append(lineNumber).append(") -> ").append(methodName).append("()").append(" ] ");
        ...
        switch (type) {
            ...
            case JSON: {
                ...
                if (jsonContent.toString().length() > 3200) {
                    Log.e(tag, "jsonContent.length = " + jsonContent.toString().length());
                    int chunkCount = jsonContent.toString().length() / 3200;
                    for (int i = 0; i <= chunkCount; i++) {
                        int max = 3200 * (i + 1);
                        if (max >= jsonContent.toString().length()) {

                            Log.e(tag, jsonContent.toString().substring(3200 * i));

                        } else {

                            Log.e(tag, jsonContent.toString().substring(3200 * i, max));
